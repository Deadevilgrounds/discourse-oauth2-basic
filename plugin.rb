  ...
  def after_authenticate(auth, existing_account: nil)
    ...
    if SiteSetting.oauth2_fetch_user_details?
      if fetched_user_details = fetch_user_details(auth['credentials']['token'], auth['uid'])
        ...
        auth['info']['image'] = "https://images.evetech.net/characters/" + fetched_user_details[:avatar].to_s + "/portrait" if fetched_user_details[:avatar]
        ...
        auth['info']['email'] = auth['info']['email'].to_s + "@eveuniversity.org"
      else
